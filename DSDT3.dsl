                ...
                Device (EC0)
                {
                    ...
                    Method (_QA4, 0, NotSerialized)  // _Qxx: EC Query
                    {
                        ...
                        If (IGDS)
                        {
                            If (LEqual (One, DDMB))
                            {
                                Store (0x0400, ^^^IGPU.NADL)
                                Store (Zero, ^^^IGPU.NDL2)
                            }

                            If (LEqual (0x02, DDMB))
                            {
                                Store (0x0100, ^^^IGPU.NADL)
                                Store (Zero, ^^^IGPU.NDL2)
                            }

                            If (LEqual (0x10, DDMB))
                            {
                                Store (0x0300, ^^^IGPU.NADL)
                                Store (Zero, ^^^IGPU.NDL2)
                            }
                            ...
                            {
                                Store (0x0400, ^^^IGPU.NADL)
                                Store (0x0100, ^^^IGPU.NDL2)
                            }

                            If (LEqual (0x11, DDMB))
                            {
                                Store (0x0400, ^^^IGPU.NADL)
                                Store (0x0300, ^^^IGPU.NDL2)
                                ^^^IGPU.GHDS (Zero)
                            }
                    ...
                    {
                        Store (Zero, Local0)
                        If (LNotEqual (^^^IGPU.CPDL, Zero))
                        {
                            Or (Local0, CDEV (^^^IGPU.CPDL), Local0)
                            Increment (TMPP)
                        }

                        If (LNotEqual (^^^IGPU.CPL2, Zero))
                        {
                            Or (Local0, CDEV (^^^IGPU.CPL2), Local0)
                            Increment (TMPP)
                        }

                        If (LNotEqual (^^^IGPU.CPL3, Zero))
                        {
                            Or (Local0, CDEV (^^^IGPU.CPL3), Local0)
                            Increment (TMPP)
                        }

                        If (LNotEqual (^^^IGPU.CPL4, Zero))
                        {
                            Or (Local0, CDEV (^^^IGPU.CPL4), Local0)
                            Increment (TMPP)
                        }

                        Store (Zero, Local1)
                        If (LNotEqual (^^^IGPU.CADL, Zero))
                        {
                            Or (Local1, CDEV (^^^IGPU.CADL), Local1)
                        }

                        If (LNotEqual (^^^IGPU.CAL2, Zero))
                        {
                            Or (Local1, CDEV (^^^IGPU.CAL2), Local1)
                        }

                        If (LNotEqual (^^^IGPU.CAL3, Zero))
                        {
                            Or (Local1, CDEV (^^^IGPU.CAL3), Local1)
                        }

                        If (LNotEqual (^^^IGPU.CAL4, Zero))
                        {
                            Or (Local1, CDEV (^^^IGPU.CAL4), Local1)
                        }
